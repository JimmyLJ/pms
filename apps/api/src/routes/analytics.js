...
const app = new Hono()
    ...
    if (!isAdmin) {
        const userProjects = await db
            ...
            .where(and(eq(projectMembers.userId, userId), eq(projects.organizationId, workspaceId)));
    ...
    const [globalOverdueResult] = await db
        ...
        .from(tasks)
        .where(and(...taskConditions, lt(tasks.dueDate, now), ne(tasks.status, "done"), ne(tasks.status, "completed")));
    // --- 个人维度统计 ---
    // 5. 我的任务总数
    ...
    const myActiveTasks = await db
        .select()
        .from(tasks)
        .where(and(...taskConditions, eq(tasks.assigneeId, userId), eq(tasks.status, "IN_PROGRESS")))
    ...
    const myOverdueTasks = await db
        .select()
        .from(tasks)
        .where(and(...taskConditions, eq(tasks.assigneeId, userId), lt(tasks.dueDate, now), ne(tasks.status, "DONE"), ne(tasks.status, "completed")))
