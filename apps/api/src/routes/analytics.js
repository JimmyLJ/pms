...
const app = new Hono()
    ...
    const [totalProjectsResult] = await db
        .select({ count: sql `cast(count(${projects.id}) as int)` })
        .from(projects)
    ...
    const taskCounts = await db
        .select({
        status: tasks.status,
        count: sql `cast(count(${tasks.id}) as int)`,
    })
        .from(tasks)
    ...
    const [globalOverdueResult] = await db
        .select({ count: sql `cast(count(${tasks.id}) as int)` })
        .from(tasks)
        .where(and(eq(tasks.organizationId, workspaceId), lt(tasks.dueDate, now), ne(tasks.status, "done"), // 假设 'done' 是完成状态，根据之前的代码可能是 'DONE' 或 'completed'，需统一。这里假设用 tasks 表里的 status
    ...
    const myActiveTasks = await db
        .select()
        .from(tasks)
        .where(and(eq(tasks.organizationId, workspaceId), eq(tasks.assigneeId, userId), eq(tasks.status, "IN_PROGRESS") // 假设数据库存的是 IN_PROGRESS
    ))
    ...
    const myOverdueTasks = await db
        .select()
        .from(tasks)
        .where(and(eq(tasks.organizationId, workspaceId), eq(tasks.assigneeId, userId), lt(tasks.dueDate, now), ne(tasks.status, "DONE"), ne(tasks.status, "completed")))
