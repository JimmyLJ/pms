...
export default function DashboardPage() {
  ...
  const { counts, lists } = data || {
    ...
    lists: {
      recentProjects: [],
      recentActivity: [],
      myStats: { inProgress: [], overdue: [], recent: [] },
    },
  };
    ...
    return (
      <div className="space-y-2">
        {tasks.map((task) => (
          <div
            key={task.id}
            className="flex items-center justify-between text-sm p-2 hover:bg-muted/50 rounded-md transition-colors cursor-pointer group"
          >
    ...
    <div className="space-y-8">
      <div>
        <h2 className="text-3xl font-bold tracking-tight">
          欢迎回来,{" "}
          {session?.user.name?.split(" ")[1] || session?.user.name || "User"}
        </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <CardContent className="pt-0 divide-y">
                {lists.recentProjects.map((project: Project) => {
                  const normalizedStatus = (project.status || "planning")
                    ...
                    .replace(/\s+/g, "_");
                  const statusLabel =
                    statusLabels[normalizedStatus] || statusLabels.planning;
                  const statusStyle =
                    statusStyles[normalizedStatus] || statusStyles.planning;

                  const progressBarStyle =
                    progressBarStyles[normalizedStatus] ||
                    progressBarStyles.planning;
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start gap-4">
                          ...
                          <div>
                            <h4 className="text-sm font-medium leading-none mb-2 mt-1">
                              {task.title}
                            </h4>
        ...
        <div className="space-y-6">
          ...
          <Card className="shadow-sm">
            ...
            <CardContent className="pb-6 pt-4">
              <MiniTaskList
                tasks={lists.myStats.inProgress}
                emptyMessage="无进行中任务"
              />
            </CardContent>
          </Card>
